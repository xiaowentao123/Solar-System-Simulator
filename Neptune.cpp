...
void Neptune::Tick(float deltaTime){

    if(isClickOn){
        glStencilFunc(GL_NOTEQUAL,10,0xFF);
        glStencilMask(0x00);
    ...
    if(!isClickOn){
        this->transform.Position.x=850*glm::cos(radians(glfwGetTime()*0.9f));
        this->transform.Position.z=850*glm::sin(radians(glfwGetTime()*0.9f));
    }
}

void Neptune::Instance(Shader &shader,FpsCamera camera,mat4 projection)
{   
...
void Neptune::Startup(){


}
vec3 Neptune::Rayhit(vec3 dir){
    mat4 Model;
    Model=translate(Model,transform.Position);
    Model=scale(Model,transform.Scale*1.01f);
    return mesh.Rayhit(this->lookCamera.Postion,dir,Model);
}
void Neptune::Instance(FpsCamera& camera,mat4 projection){
