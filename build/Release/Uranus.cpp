...
void Uranus::Tick(float deltaTime){

    if(isClickOn){
        glStencilFunc(GL_NOTEQUAL,9,0xFF);
        glStencilMask(0x00);
    ...
    if(!isClickOn){
        this->transform.Position.x=750*glm::cos(radians(glfwGetTime()*1.f));
        this->transform.Position.z=750*glm::sin(radians(glfwGetTime()*1.f));
    }
}

void Uranus::Instance(Shader &shader,FpsCamera camera,mat4 projection)
{   
