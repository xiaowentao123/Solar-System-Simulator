...
void Venus::Tick(float deltaTime){
    ...
    if(!isClickOn){
        this->transform.Position.x=150*glm::cos(radians(glfwGetTime()*4));
        this->transform.Position.z=150*glm::sin(radians(glfwGetTime()*4));
    }
}

void Venus::Instance(Shader &shader,FpsCamera camera,mat4 projection)
{   
