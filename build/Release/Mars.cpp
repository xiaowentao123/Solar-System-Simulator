...
void Mars::Tick(float deltaTime){

    if(isClickOn){
        glStencilFunc(GL_NOTEQUAL,6,0xFF);
        glStencilMask(0x00);
    ...
    if(!isClickOn){
        this->transform.Position.x=300*glm::cos(radians(glfwGetTime()*1.5));
        this->transform.Position.z=300*glm::sin(radians(glfwGetTime()*1.5));
    }
}

void Mars::Instance(Shader &shader,FpsCamera camera,mat4 projection)
{   
...
void Mars::Instance(FpsCamera& camera,mat4 projection){
    ...
    Model=rotate(Model,radians(transform.Rotation.y),vec3(0.f,1.f,0.f));
    Model=rotate(Model,radians(transform.Rotation.x),vec3(1.f,0.f,0.f));
    Model=rotate(Model,radians(30.f),vec3(0.f,0.f,1.f));
