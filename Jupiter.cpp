...
void Jupiter::Tick(float deltaTime){

    if(isClickOn){
        glStencilFunc(GL_NOTEQUAL,7,0xFF);
        glStencilMask(0x00);
    ...
    if(!isClickOn){
        this->transform.Position.x=550*glm::cos(radians(glfwGetTime()*1.3));
        this->transform.Position.z=550*glm::sin(radians(glfwGetTime()*1.3));
    }
}

void Jupiter::Instance(Shader &shader,FpsCamera camera,mat4 projection)
{   
...
void Jupiter::Startup(){


}
vec3 Jupiter::Rayhit(vec3 dir){
    mat4 Model;
    Model=translate(Model,transform.Position);
    Model=scale(Model,transform.Scale*1.01f);
    return mesh.Rayhit(this->lookCamera.Postion,dir,Model);
}
void Jupiter::Instance(FpsCamera& camera,mat4 projection){
    ...
    Model=rotate(Model,radians(-20.f),vec3(0.f,0.f,1.f));
    Model=rotate(Model,radians(-78.f),vec3(1.f,0.f,0.f));
    Model=scale(Model,transform.Scale);
    shader.setMat4("Model",Model);
    shader.setMat4("View",camera.getLooAt());
    shader.setMat4("Projection",projection);
